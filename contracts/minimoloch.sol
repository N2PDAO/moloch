...
contract Moloch {
    ...
    function add_member(address new_applicant)public{
      members[new_applicant] = Member(new_applicant, 20, 0, true, 0,new address[](0),false);
    ...
    function submitVote(uint256 proposalIndex, uint8 uintVote) public {
      ...
      //  require(!hasVotingPeriodExpired(proposal.startingPeriod), "Moloch::submitVote - proposal voting period has expired");
        ...
        for (uint i=0; i< member.addressDelegatedTo.length; i++) {
            ...
            }else if (proposal.votesByMember[voted] == Vote.Yes || vote == Vote.No){
                uint256 shares =  members[voted].sharesDelegated[msg.sender];
                correction_factor = correction_factor.add(shares);
            }
        }

        // count vote
        if (vote == Vote.Yes) {
            proposal.yesVotes = proposal.yesVotes.add(member.shares + member.delegatedShares - correction_factor );
            ...
            proposal.noVotes = proposal.noVotes.add(member.shares + member.delegatedShares - correction_factor);
        }
